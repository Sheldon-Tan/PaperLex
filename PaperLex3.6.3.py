# PaperLex - 学术论文精读桌面词典 (支持高清 LaTeX 公式光栅化、Markdown 原生表格与代码块渲染、表格自动去重)
import datetime
import io
import json
import os
...
def open_settings_dialog(parent_window):
    ...
    lbl_hint = tk.Label(dialog, text="• 仅保存在本地 config.json，留空不影响基础查词", font=("微软雅黑", 8), fg="#64748B", bg="#F8FAFC")
    ...
    btn_ok = tk.Button(btn_frame, text="保存设置", font=("微软雅黑", 8, "bold"), bg="#2563EB", fg="white",
                       relief="flat", activebackground="#1D4ED8", activeforeground="white", cursor="hand2", padx=12, pady=3, command=save_and_close)
...
def extract_base_form(data: dict, raw_trans: list, clean_text: str):
    ...
    # 1. 大小写智能逆推
    if clean_text != clean_w and clean_text[0].isupper() and not clean_text.isupper():
        lower_info = query_youdao(clean_w)
        if lower_info and (lower_info.get("ec") or lower_info.get("simple")):
