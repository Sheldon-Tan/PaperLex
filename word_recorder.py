...
def fetch_entry_info(text: str):
    ...
    # 清洗括号 (如 "Kinda (kind of)")
    clean_text = re.sub(r'[\(（].*?[\)）]', '', raw_text).strip()
    if not clean_text:
        clean_text = raw_text

    # 斜杠多词合并 (如 "Phonemes / Visemes")
    if "/" in clean_text:
...
def save_to_excel(info: dict):
    ...
    try:
        ...
        wb.save(EXCEL_FILE)
        print(f"✨ 已成功将「{info['word']}」保存到 {EXCEL_FILE}（已自动加粗排版）\n")
...
def main():
    ...
    print(" • 特色：第一列自动粗体排版 | 自动兼容大小写 | 保存到 words.xlsx")
    ...
    while True:
        ...
        if is_word_already_saved(user_input):
            print(f"💡 提示：该词条之前已经记录在 {EXCEL_FILE} 中，本次将继续查询并追加。")
