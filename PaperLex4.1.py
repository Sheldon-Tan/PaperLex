# PaperLex - 学术论文精读桌面词典 (集成牛津/柯林斯权威英汉双解搜索、网页搜索双模态切换、LaTeX 渲染、自动去重与截断逆推)
...
# --- Matplotlib 中文字体与渲染配置 ---
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'DejaVu Sans']
plt.rcParams['axes.unicode_minus'] = False
plt.rcParams['mathtext.fontset'] = 'stixsans'  # 兼容 CJK 字符与无衬线数学符号
...
DEFAULT_CONFIG = {
    ...
    "model": "deepseek-chat",
    "auto_dedup": True  # 默认开启表格自动去重 (新记录覆盖旧记录)
}
...
def open_settings_dialog(parent_window):
    """可视化设置窗口：支持 API Key 掩码与表格自动去重开关配置"""
    cfg = load_or_init_config()

    dialog = tk.Toplevel(parent_window)
    dialog.title("设置 - PaperLex")
    ...
    try:
        x = parent_window.winfo_x() + (parent_window.winfo_width() - 380) // 2
        y = parent_window.winfo_y() + (parent_window.winfo_height() - 230) // 2
        dialog.geometry(f"+{x}+{y}")
    except Exception:
        pass

    # 1. API Key 配置
    lbl_key = tk.Label(dialog, text="DeepSeek API Key:", font=("微软雅黑", 9, "bold"), fg="#1E293B", bg="#F8FAFC")
    lbl_key.pack(anchor="w", padx=16, pady=(14, 4))
    ...
    entry_key = tk.Entry(dialog, textvariable=key_var, show="*", font=("微软雅黑", 9), relief="flat",
                         highlightthickness=1, highlightbackground="#CBD5E1", highlightcolor="#2563EB", bg="#FFFFFF")
    entry_key.pack(fill="x", padx=16, ipady=3)

    lbl_hint = tk.Label(dialog, text="• 仅保存在本地 config.json，留空不影响基础查词与双解检索", font=("微软雅黑", 8),
                        fg="#64748B", bg="#F8FAFC")
    lbl_hint.pack(anchor="w", padx=16, pady=(2, 10))

    # 2. 自动去重开关
    dedup_var = tk.BooleanVar(value=cfg.get("auto_dedup", True))
    chk_dedup = tk.Checkbutton(
        dialog,
        text="表格自动去重 (同词重复保存时，用新记录覆盖旧记录)",
        variable=dedup_var,
        font=("微软雅黑", 8, "bold"),
        fg="#1E293B",
        bg="#F8FAFC",
    ...
    chk_dedup.pack(anchor="w", padx=16, pady=(0, 14))

    # 3. 操作按钮区
    btn_frame = tk.Frame(dialog, bg="#F8FAFC")
    btn_frame.pack(fill="x", padx=16, side="bottom", pady=12)

    def save_and_close():
        new_key = key_var.get().strip()
        cfg["api_key"] = new_key
        cfg["enabled"] = bool(new_key)
        cfg["auto_dedup"] = dedup_var.get()
        if save_config(cfg):
            dialog.destroy()
            messagebox.showinfo("成功", "配置已成功保存！", parent=parent_window)

    btn_cancel = tk.Button(btn_frame, text="取消", font=("微软雅黑", 8), bg="#E2E8F0", fg="#475569",
                           relief="flat", cursor="hand2", padx=12, pady=3, command=dialog.destroy)
    btn_cancel.pack(side="right", padx=(6, 0))

    btn_ok = tk.Button(btn_frame, text="保存设置", font=("微软雅黑", 8, "bold"), bg="#2563EB", fg="white",
                       relief="flat", activebackground="#1D4ED8", activeforeground="white", cursor="hand2", padx=12,
                       pady=3, command=save_and_close)
...
# --- Free Translation Engine (Long Sentence & Fast Batch) ---

def translate_text(text: str) -> str:
    """轻量免费翻译引擎：采用移动端专用通道，彻底解决 errorCode:50 报错"""
    clean_text = text.strip()
    if not clean_text:
        return ""

    # 通道 1: 有道纯净翻译通道 (dicts=[["fanyi"]])，无 anti-crawler 阻断
    try:
        url = "https://dict.youdao.com/jsonapi"
        payload = {
            "client": "mobile",
            "q": clean_text,
            "dicts": '{"count": 1, "dicts": [["fanyi"]]}'
        }
        res = requests.post(url, data=payload, headers=HEADERS, proxies=NO_PROXY, timeout=4)
        if res.status_code == 200:
            data = res.json()
            fanyi = data.get("fanyi", {}).get("tran")
            if fanyi and fanyi.strip().lower() != clean_text.lower():
                return fanyi.strip()
    except Exception:
        pass
    ...
    try:
        ...
        res = requests.get(
            url,
            params={"client": "mobile", "q": clean_text, "dicts": '{"count": 1, "dicts": [["fanyi"]]}'},
            headers=HEADERS,
            proxies=NO_PROXY,
            timeout=4
        )
    ...
    try:
        ...
        res = requests.get(
            url,
            params={"q": clean_text[:250], "langpair": "en|zh-CN"},
            headers=HEADERS,
            proxies=NO_PROXY,
            timeout=4
        )
        if res.status_code == 200:
            ...
            if trans and trans.strip().lower() != clean_text.lower():
                return trans.strip()
    except Exception:
        pass
...
def batch_translate_texts(texts: list) -> list:
    """多线程并发翻译牛津英文释义与例句，确保在最短时间内返回"""
    if not texts:
        return []
    with ThreadPoolExecutor(max_workers=min(len(texts), 6)) as executor:
        return list(executor.map(translate_text, texts))


# --- Ultra-HD LaTeX & Markdown Rich Text Engine ---

def sanitize_latex_for_mathtext(latex_expr: str) -> str:
    """全面清洗并修复 MathText 不支持的宏、双引号及伪代码环境"""
    clean_expr = latex_expr.strip()
    clean_expr = re.sub(r'^(?:\\\[|\$\$|\\\(|\$)|(?:\\\]|\$\$|\\\)|\$)$', '', clean_expr).strip()

    clean_expr = clean_expr.replace('"', "''")
    clean_expr = clean_expr.replace(r'\textbf{', r'\mathbf{')
    clean_expr = clean_expr.replace(r'\textit{', r'\mathit{')
    clean_expr = clean_expr.replace(r'\begin{aligned}', r'\begin{matrix}').replace(r'\end{aligned}', r'\end{matrix}')
...
def fallback_clean_math_text(raw_math: str) -> str:
    """当公式渲染彻底失败时，剥离 LaTeX 语法外壳作为优雅纯文本引用显示"""
    text = re.sub(r'^(?:\\\[|\$\$|\\\(|\$)|(?:\\\]|\$\$|\\\)|\$)$', '', raw_math.strip()).strip()
    text = re.sub(r'\\(?:text|mathrm|mathbf|textbf|mathit)\{([^}]+)\}', r'\1', text)
    text = text.replace(r'\begin{aligned}', '').replace(r'\end{aligned}', '')
    text = text.replace(r'\_', '_').replace(r'\ ', ' ').replace(r'\quad', '    ').replace(r'\\', '\n')
    return text.strip()
...
def setup_rich_text_tags(text_widget: tk.Text):
    ...
    text_widget.tag_configure("h4", font=("微软雅黑", 8, "bold"), foreground="#475569", spacing1=3, spacing3=1)
    ...
    text_widget.tag_configure("quote", font=("微软雅黑", 8, "italic"), foreground="#475569")
    text_widget.tag_configure("inline_code", font=("Consolas", 8), background="#F1F5F9", foreground="#0F172A")
...
def parse_inline_tokens(text_widget: tk.Text, line_str: str, base_tag: str = "normal"):
    ...
    for part in parts:
        ...
        if part.startswith("**") and part.endswith("**") and len(part) >= 4:
            ...
            text_widget.insert(tk.END, f" {part[1:-1]} ", "inline_code")
...
def render_markdown_table(text_widget: tk.Text, table_lines: list):
    ...
    text_widget._sub_widgets.append(tbl_frame)
...
def render_code_block(text_widget: tk.Text, code_text: str):
    """将代码块渲染为优雅的独立等宽文本框容器"""
    clean_code = code_text.strip()
    if not clean_code:
        return

    code_frame = tk.Frame(text_widget, bg="#F1F5F9", bd=1, relief="solid", padx=6, pady=4)
    text_widget._sub_widgets.append(code_frame)
    ...
    lbl_code.pack(fill="both", expand=True)

    text_widget.insert(tk.END, "\n")
    text_widget.window_create(tk.END, window=code_frame)
    text_widget.insert(tk.END, "\n\n")


def render_markdown_with_latex(text_widget: tk.Text, raw_text: str):
    """Markdown 语法、代码块、原生表格与 LaTeX 公式的完整图文混排解析器"""
    if not hasattr(text_widget, "_images_cache"):
        text_widget._images_cache = []
    if not hasattr(text_widget, "_sub_widgets"):
        text_widget._sub_widgets = []

    text_widget._images_cache.clear()
    text_widget._sub_widgets.clear()
    setup_rich_text_tags(text_widget)

    master_pattern = r'(```[\s\S]*?```|\\\[[\s\S]*?\\\]|\$\$[\s\S]*?\$\$|\$(?:\\begin\{[a-z]+\})[\s\S]*?(?:\\end\{[a-z]+\})\$)'
    sections = re.split(master_pattern, raw_text)

    for section in sections:
        if not section:
            continue

        if section.startswith("```") and section.endswith("```") and len(section) >= 6:
            code_content = re.sub(r'^```[a-zA-Z]*\n?', '', section)
            code_content = re.sub(r'\n?```$', '', code_content)
            render_code_block(text_widget, code_content)
            continue

        is_block_math = (
                (section.startswith(r"\[") and section.endswith(r"\]")) or
                (section.startswith("$$") and section.endswith("$$")) or
                (section.startswith("$") and section.endswith("$") and r"\begin" in section)
        )
        ...
        if is_block_math:
            try:
                ...
                fallback_txt = fallback_clean_math_text(section)
            ...
            for idx, line in enumerate(lines):
                ...
                if trimmed.startswith("---") or trimmed.startswith("___"):
                    ...
                    tag_name = f"h{min(level, 4)}"
                    parse_inline_tokens(text_widget, content, base_tag=tag_name)
                    text_widget.insert(tk.END, "\n")
...
def save_to_excel(info: dict) -> tuple[bool, bool]:
    """保存至 Excel。返回值: (是否成功, 是否为覆盖更新)"""
    cfg = load_or_init_config()
    auto_dedup = cfg.get("auto_dedup", True)
    clean_target = info["word"].strip().lower()
    ...
    try:
        with excel_lock:
            ...
            if target_row_idx is not None:
                ...
                ws.cell(row=target_row, column=1, value=info["word"])
                ws.cell(row=target_row, column=2, value=info["phonetic"])
                ws.cell(row=target_row, column=3, value=info["pos"])
                ws.cell(row=target_row, column=4, value=info["definition"])
                ws.cell(row=target_row, column=5, value=now)
            else:
                ws.append([
                    info["word"],
                    info["phonetic"],
            ...
            ws.cell(row=target_row, column=1).font = Font(name="微软雅黑", size=10, bold=True)
            ws.cell(row=target_row, column=1).alignment = Alignment(wrap_text=True, vertical="center")
            ws.cell(row=target_row, column=4).font = Font(name="微软雅黑", size=10)
            ws.cell(row=target_row, column=4).alignment = Alignment(wrap_text=True, vertical="center")
            for col_idx in [2, 3, 5]:
                ws.cell(row=target_row, column=col_idx).font = Font(name="微软雅黑", size=10)
...
def extract_base_form(data: dict, raw_trans: list, clean_text: str):
    """多维形态学与截断逆推引擎"""
    if not data and not clean_text:
        return None, None
    clean_w = clean_text.strip().lower()

    # 1. 截断词拼合逆推 (如 "Cor respondence" -> "correspondence")
    if " " in clean_text or "-" in clean_text:
        merged_candidate = re.sub(r'[\s\-]+', '', clean_text).lower()
        if len(merged_candidate) >= 3 and merged_candidate != clean_w:
            merged_info = query_youdao(merged_candidate)
            if merged_info:
                ec_data = merged_info.get("ec", {}).get("word", [{}])[0] if merged_info.get("ec") else {}
                trs_list = ec_data.get("trs", [])
                if trs_list:
                    return merged_candidate, "截断拼合"

    # 2. 单字大小写智能逆推
    if " " not in clean_text and clean_text != clean_w and clean_text[0].isupper() and not clean_text.isupper():
        lower_info = query_youdao(clean_w)
        if lower_info:
            ec_data = lower_info.get("ec", {}).get("word", [{}])[0] if lower_info.get("ec") else {}
            if ec_data.get("trs"):
                return clean_w, "小写常用词"
...
# --- Oxford / Collins Authentic Dual-Language Engine ---

def extract_bilingual_dictionary(data: dict, word: str):
    """优先提取官方权威双解词库（Collins / Oxford），提供专家精校释义与例句翻译"""
    if not data:
        return None

    # 提取 Collins COBUILD 高阶双解
    collins_data = data.get("collins")
    if collins_data:
        entries = collins_data.get("collins_entries", [])
        if entries:
            phonetic = entries[0].get("phonetic", "-")
            ...
            for entry_item in entries:
                entry_obj = entry_item.get("entries", {}).get("entry", [])
                if isinstance(entry_obj, dict):
                    entry_obj = [entry_obj]

                for sub_e in entry_obj:
                    tran_entries = sub_e.get("tran_entry", [])
                    if isinstance(tran_entries, dict):
                        tran_entries = [tran_entries]

                    for te in tran_entries:
                        pos = te.get("pos_entry", {}).get("pos", "")
                        if pos and pos not in main_pos_set:
                            main_pos_set.append(pos)

                        d_en = te.get("def", "").strip()
                        d_cn = te.get("tran", "").strip()

                        if not d_en and not d_cn:
                            continue

                        # 清洗 HTML 标签
                        d_en = html.unescape(re.sub(r'<[^>]+>', '', d_en)).strip()
                        d_cn = html.unescape(re.sub(r'<[^>]+>', '', d_cn)).strip()

                        pos_prefix = f"**[{pos}]** " if pos else ""
                        block = f"### {sense_idx}. {pos_prefix}{d_en}"
                        if d_cn:
                            block += f"\n- **中文释义**：{d_cn}"

                        # 提取例句与例句中文翻译
                        exam_sents = te.get("exam_sents", {}).get("sent", [])
                        if isinstance(exam_sents, dict):
                            exam_sents = [exam_sents]

                        for ex in exam_sents[:2]:
                            eng = html.unescape(re.sub(r'<[^>]+>', '', ex.get("eng_sent", ""))).strip()
                            chn = html.unescape(re.sub(r'<[^>]+>', '', ex.get("chn_sent", ""))).strip()
                            if eng:
                                block += f"\n> *eg: {eng}*"
                                if chn:
                                    block += f"\n> *(例：{chn})*"

                        senses_output.append(block)
                        sense_idx += 1
                        if sense_idx > 8:
                            break
                    if sense_idx > 8:
            ...
            if senses_output:
                base_w, inf_type = extract_base_form(data, [], word)
                pos_display = "/".join(main_pos_set[:3]) if main_pos_set else "双解"
                return {
                    "word": entries[0].get("headword", word),
                    "phonetic": phonetic,
                    "pos": f"[牛津双解·{pos_display}]",
                    "definition": "\n\n".join(senses_output),
                    "is_uncertain": False,
                    "base_form": base_w,
                    "inflection_type": inf_type,
                    "verb_forms": extract_verb_forms(data),
                    "memory": ""
                }
...
def fetch_oxford_entry_info(text: str):
    """牛津/双解模式统一检索调度器"""
    clean_w = text.strip()
    if not clean_w or is_contains_chinese(clean_w):
        return None

    # 1. 优先提取官方权威双解词库（零额外网络延迟，100% 精准中文）
    yd_data = query_youdao(clean_w)
    if yd_data:
        bilingual_info = extract_bilingual_dictionary(yd_data, clean_w)
        if bilingual_info:
            return bilingual_info

    # 2. 回退方案：在线爬取 Oxford 官网英文并由多通道翻译引擎补齐中文释义与例句
    query_target = re.sub(r'[\s\-]+', '', clean_w) if (" " in clean_w and not clean_w.isupper()) else clean_w
    url = f"https://www.oxfordlearnersdictionaries.com/search/english/?q={quote(query_target)}"
    ...
    try:
        resp = requests.get(url, headers=headers, proxies=NO_PROXY, timeout=8)
        if resp.status_code != 200:
            return None
        html_text = resp.text

        if "No dictionary results" in html_text or "did not match any dictionary entries" in html_text:
            return None

        headword = query_target
        pos_str = "-"
        phonetic_str = "-"
        raw_senses = []
        ...
        try:
            ...
            hw_elem = soup.find("h1", class_="headword")
            if hw_elem:
                headword = hw_elem.get_text().strip()

            pos_elem = soup.find("span", class_="pos")
            if pos_elem:
                pos_str = pos_elem.get_text().strip()

            phon_parts = []
            uk_div = soup.find("div", class_="phons_br")
            if uk_div:
                phon = uk_div.find("span", class_="phon")
                if phon:
                    phon_parts.append(f"英 /{phon.get_text().strip('/')}/")
            us_div = soup.find("div", class_="phons_n_am")
            if us_div:
                phon = us_div.find("span", class_="phon")
                if phon:
                    phon_parts.append(f"美 /{phon.get_text().strip('/')}/")
            phonetic_str = "  ".join(phon_parts) if phon_parts else "-"
            ...
            for s in senses[:6]:
                ...
                if not def_span:
                    continue
                d_text = def_span.get_text().strip()
                ex_span = s.find("span", class_="x")
                ex_text = ex_span.get_text().strip() if ex_span else ""
                raw_senses.append({"def": d_text, "ex": ex_text})
        except Exception:
            hw_m = re.search(r'<h1 class="headword"[^>]*>(.*?)</h1>', html_text, re.DOTALL)
            headword = html.unescape(re.sub(r'<[^>]+>', '', hw_m.group(1))).strip() if hw_m else query_target

            pos_m = re.search(r'<span class="pos"[^>]*>(.*?)</span>', html_text, re.DOTALL)
            pos_str = html.unescape(re.sub(r'<[^>]+>', '', pos_m.group(1))).strip() if pos_m else "-"

            uk_m = re.search(r'<div class="phons_br"[^>]*>.*?<span class="phon"[^>]*>(.*?)</span>', html_text, re.DOTALL)
            us_m = re.search(r'<div class="phons_n_am"[^>]*>.*?<span class="phon"[^>]*>(.*?)</span>', html_text, re.DOTALL)
            phon_parts = []
            if uk_m:
                phon_parts.append(f"英 /{html.unescape(re.sub(r'<[^>]+>', '', uk_m.group(1))).strip('/')}/")
            if us_m:
                phon_parts.append(f"美 /{html.unescape(re.sub(r'<[^>]+>', '', us_m.group(1))).strip('/')}/")
            phonetic_str = "  ".join(phon_parts) if phon_parts else "-"

            senses_matches = re.findall(r'<li class="sense"[^>]*>(.*?)</li>\s*(?=<li class="sense"|</ul>|</div>)', html_text, re.DOTALL)
            for sense_html in senses_matches[:6]:
                def_m = re.search(r'<span class="def"[^>]*>(.*?)</span>', sense_html, re.DOTALL)
                if not def_m:
                    continue
                d_clean = html.unescape(re.sub(r'<[^>]+>', '', def_m.group(1))).strip()
                ex_matches = re.findall(r'<span class="x"[^>]*>(.*?)</span>', sense_html, re.DOTALL)
                ex_clean = html.unescape(re.sub(r'<[^>]+>', '', ex_matches[0])).strip() if ex_matches else ""
                raw_senses.append({"def": d_clean, "ex": ex_clean})
        ...
        for item in raw_senses:
            texts_to_translate.append(item["def"])
            if item["ex"]:
                texts_to_translate.append(item["ex"])

        translations = batch_translate_texts(texts_to_translate)
        t_iter = iter(translations)

        def_lines = []
        for idx, item in enumerate(raw_senses, 1):
            d_cn = next(t_iter, "").strip()
            ex_cn = next(t_iter, "").strip() if item["ex"] else ""

            block = f"### {idx}. {item['def']}"
            if d_cn and d_cn.lower() != item['def'].lower():
                block += f"\n- **中文释义**：{d_cn}"
            if item["ex"]:
                block += f"\n> *eg: {item['ex']}*"
                if ex_cn and ex_cn.lower() != item['ex'].lower():
                    block += f"\n> *(例：{ex_cn})*"
            def_lines.append(block)

        base_w, inf_type = extract_base_form(yd_data or {}, [], clean_w)
...
def call_llm_direct_query(text: str, context_hint: str = "") -> str:
    ...
    prompt = (
        ...
        f"【公式与代码排版硬性约束】：\n"
        f"1. 【严禁将伪代码或算法流程用 LaTeX 公式 (如 \\begin{{aligned}}, \\textbf{{while}}) 包装】；所有算法步骤、伪代码及代码片段必须使用标准 Markdown 代码块（```text ... ```）排版。\n"
        f"2. 仅对真实的代数、微积分、损失函数使用 \\[ 与 \\] 包装；严禁在公式内使用中文字符与双引号 \"。\n"
        f"3. 行内简短数学变量使用 \\( 与 \\) 包裹。\n\n"
        f"请自适应判断目标类型并输出学术干货：\n\n"
        f"【类型 A：技术任务 / 前沿概念 / 专业缩写（如 NMS, Active Speaker Detection, CLIP, LoRA, DETR 等）】\n"
        f"### 1. 📌 标准中文术语名\n学术界公认中文规范定名与英文全称。\n\n"
        f"### 2. 🔬 核心技术机理\n输入输出模态、损失函数/核心数学原理，以及标准算法流程（伪代码请用 ```text 呈现）。\n\n"
...
def fetch_english_candidates_from_chinese(chinese_text: str) -> list:
    free_tran = translate_text(chinese_text)
    cfg = load_or_init_config()
    api_key = cfg.get("api_key", "").strip()
    ...
    base_url = cfg.get("base_url", "[https://api.deepseek.com](https://api.deepseek.com)").rstrip("/")
    endpoint = f"{base_url}/chat/completions" if base_url.endswith("/v1") else f"{base_url}/v1/chat/completions"
...
def fetch_entry_info(text: str):
    ...
    candidates = []
    if len(clean_text) > 1 and clean_text[0].isupper() and not clean_text.isupper():
    ...
    for cand in candidates:
        ...
        if res and (res.get("ec") or res.get("fanyi") or res.get("web_trans") or res.get("simple")):
            ec_word = res.get("ec", {}).get("word", [{}])[0] if res.get("ec") else {}
            trs_text = "".join([str(t) for t in ec_word.get("trs", [])])
            if "人名" in trs_text and len(ec_word.get("trs", [])) <= 1 and cand != clean_text.lower():
                lower_res = query_youdao(clean_text.lower())
                if lower_res and lower_res.get("ec"):
                    data = lower_res
                    break
            data = res
            break
    ...
    if len(split_tokens) >= 2 and (not data or (not data.get("ec") and not data.get("web_trans"))):
        ...
        if sub_words_info:
            free_meaning = translate_text(clean_text.replace("-", " "))
            ...
            base_form, inflection_type = extract_base_form(data or {}, [], clean_text)
    ...
    if not data:
        tran_text = translate_text(clean_text)
        if tran_text:
            base_form, inflection_type = extract_base_form({}, [], clean_text)
            return {
                "word": raw_text,
                "phonetic": "-",
                "pos": "[短语/长句]",
    ...
    memory_str = extract_associative_memory(data, clean_text.lower())
...
class FloatingLookupApp:
    def __init__(self):
        ...
        self.cached_web_info = None
        self.cached_oxford_info = None
    ...
    def _build_ui(self):
        ...
        # --- 有道风格选项卡栏 (网页搜索 / 牛津搜索) ---
        self.tab_frame = tk.Frame(self.root, bg="#F8FAFC", padx=8, pady=2)
        self.tab_frame.pack(fill="x", side="top")

        self.btn_tab_web = tk.Button(
            self.tab_frame,
            text="🌐 网页搜索",
            font=("微软雅黑", 8, "bold"),
            relief="flat",
            ...
            padx=10,
            pady=2,
            command=lambda: self.switch_mode("web")
        )
        self.btn_tab_web.pack(side="left", padx=(0, 4))

        self.btn_tab_oxford = tk.Button(
            self.tab_frame,
            text="📖 牛津搜索",
            font=("微软雅黑", 8),
            relief="flat",
            ...
            padx=10,
            pady=2,
            command=lambda: self.switch_mode("oxford")
        )
        self.btn_tab_oxford.pack(side="left")
    ...
    def _update_tab_styles(self):
        if self.search_mode == "web":
            self.btn_tab_web.config(bg="#FFFFFF", fg="#2563EB", font=("微软雅黑", 8, "bold"), relief="solid", bd=1)
            self.btn_tab_oxford.config(bg="#F1F5F9", fg="#64748B", font=("微软雅黑", 8), relief="flat", bd=0)
        else:
            self.btn_tab_web.config(bg="#F1F5F9", fg="#64748B", font=("微软雅黑", 8), relief="flat", bd=0)
            self.btn_tab_oxford.config(bg="#FFFFFF", fg="#2563EB", font=("微软雅黑", 8, "bold"), relief="solid", bd=1)

    def switch_mode(self, new_mode: str):
        if self.search_mode == new_mode:
            return
        self.search_mode = new_mode
        self._update_tab_styles()

        target = self.entry_var.get().strip() or self.last_query_text
        if not target:
            return

        if self.search_mode == "web" and self.cached_web_info and self.cached_web_info["word"].lower() == target.lower():
            self.display_english_result(self.cached_web_info)
        elif self.search_mode == "oxford" and self.cached_oxford_info and self.cached_oxford_info["word"].lower() == target.lower():
            self.display_english_result(self.cached_oxford_info)
        else:
    ...
    def display_english_result(self, info, suggestions=None):
        ...
        if is_word_already_saved(info["word"]):
            if cfg.get("auto_dedup", True):
                self.lbl_status.config(text="已在生词本中 (按 Enter 覆盖更新)", fg="#D97706")
            else:
    ...
    def start_lookup(self, text: str):
        ...
        if self.last_query_text.lower() != clean_text.lower():
            self.cached_web_info = None
            self.cached_oxford_info = None
            self.last_query_text = clean_text

        engine_name = "网页搜索" if self.search_mode == "web" else "牛津双解"
        self.set_loading(clean_text, custom_msg=f"正在通过 [{engine_name}] 检索...")
        threading.Thread(target=self._async_lookup_worker, args=(clean_text, self.search_mode), daemon=True).start()
    ...
    def _async_lookup_worker(self, text: str, mode: str):
        ...
        if mode == "oxford":
            info = fetch_oxford_entry_info(text)
            if info:
                self.cached_oxford_info = info
                self.root.after(0, lambda: self.display_english_result(info))
            else:
                self.root.after(0, lambda: self._show_not_found(text, source_name="牛津双解"))
        else:
            ...
            if not info:
                if suggestions:
                    ...
                    self.root.after(0, lambda: self._show_not_found(text, source_name="网页词典"))
            else:
                self.cached_web_info = info
    ...
    def _show_not_found(self, text: str, source_name: str = "词典"):
        self.lbl_word.config(text=text, fg="#DC2626")
        self.lbl_sub.config(text=f"[{source_name}] 未收录，可尝试切换选项卡或点右下「深度解析」", fg="#DC2626")
    ...
    def save_current_word(self):
        if not self.current_info:
            return
        success, is_update = save_to_excel(self.current_info)
        if success:
            msg = f"已覆盖更新至表格 ({EXCEL_FILE})" if is_update else f"已保存至表格 ({EXCEL_FILE})"
            self.lbl_status.config(text=msg, fg="#059669")
...
def trigger_f8_hotkey():
    ...
    try:
        ...
        if selected_text:
            cleaned_selection = re.sub(r'(\w+)-\s*\r?\n\s*(\w+)', r'\1\2', selected_text)
            cleaned_selection = re.sub(r'\r?\n', ' ', cleaned_selection)
            cleaned_selection = re.sub(r'\s+', ' ', cleaned_selection).strip()

            app_instance.root.after(0, lambda: app_instance.start_lookup(cleaned_selection))
        else:
